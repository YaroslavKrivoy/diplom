...
define([
  ...
  return Class.extend({
    ...
    initObservable: function () {
      ...
      self.$selector.find('.submit-button').removeAttr('onclick');
      self.$selector.find('.submit-button').on('click', this.submit.bind(this));
    ...
    submit: function (e) {
      ...
      $.when(this.createToken()).done(function(result) {
        ...
        $('#' + self.container).find('#pmclain_authorizenetcim_cc_last4').val(container.find('#' + self.code + '_cc_number').val().slice(-4));
    ...
    createToken: function() {
      ...
      this.acceptjs.dispatchData(secureData, function(response) {
        if (response.messages.resultCode === "Error") {
          ...
          $('#' + self.container).find('#pmclain_authorizenetcim_cc_token').val(response.opaqueData.dataValue);
