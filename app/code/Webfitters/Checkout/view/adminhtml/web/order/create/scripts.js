    ...
    AdminOrder.prototype = {
        ...
        bindAddressFields : function(container) {
            ...
            for(var i=0;i<fields.length;i++){
                Event.observe(fields[i], 'change', this.updateAddressField.bind(this));
            }
        },

        updateAddressField: function(event){
            var field = Event.element(event);
            if(this.isBillingField(field.id) && this.shippingAsBilling){
                var name = field.name.replace('billing_address', 'shipping_address');
                jQuery('[name="' + name + '"]').val(jQuery(field).val());
            }
        },
        ...
        changeAddressField: function () {
            ...
            /*} else {*/
                data2 = this.serializeData(this.shippingAddressContainer);
                Object.keys(data2._object).forEach(function(item){
                    data._object[item] = data2._object[item];
                });
            /*}*/
            ...
            data['order[billing_address][customer_address_id]'] = null;
            data['shipping_as_billing'] = jQuery('[name="shipping_same_as_billing"]').is(':checked') ? 1 : 0;
            data['order[billing_address][customer_address_id]'] = $('order-billing_address_customer_address_id').value;
            data['order[shipping_address][customer_address_id]'] = $('order-shipping_address_customer_address_id').value;
        ...
        setShippingAsBilling : function(flag){
            ...
            console.log();
            var elements = this.serializeData(this.billingAddressContainer).toObject();
            if(flag){
                Object.keys(elements).forEach(function(item){
                    var name = item.replace('billing_address', 'shipping_address');
                    jQuery('[name="'+name+'"]').val(elements[item]);
                });
            }
        ...
        productGridRowClick : function(grid, event){
            ...
            var isInputWgt = eventElement.tagName == 'INPUT' && eventElement.name == 'qty_weight';
            if (trElement && !isInputQty && !isInputWgt) {
        ...
        productGridAddSelected : function(){
            ...
            for (var productId in products) {
                ...
                for (var productParamKey in products[productId]) {
                    var newKey = paramKey;
                    newKey += '['+productParamKey+']';
                    fieldsPrepare[newKey] = products[productId][productParamKey];
                }
            }
