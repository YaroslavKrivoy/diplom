...
class Append
{
    ...
    public function beforeAppend($subject, $result)
    {
        ...
        if ($this->state->getAreaCode() == \Magento\Framework\App\Area::AREA_ADMINHTML && !(php_sapi_name() === 'cli')) {
            try {
                $customer = $this->customer->getById($this->session->getCustomerId());
                $customerGroup = $customer->getGroupId();
            } catch(\Exception $e){$customerGroup = 0;}
        } else {
