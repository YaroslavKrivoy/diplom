    ...
    function ($) {
        ...
        $.widget('mage.mageworxOrderEditorItemsForm', {
            ...
            updateInputValues:function (item) {
                ...
                switch (name) {
                    ...
                    case "fact_qty":
                        ...
                        break;
                    case "fact_cases":
                        this.checkFactCases(input);
                        break;
                    case "fact_unit":
                        this.checkFactUnit(input);
                        this.calculateSubtotal(input);
                        break;
                    case "tax_amount":
                        break;
            ...
            checkFactQty: function (item) {
                ...
                var weight_per_unit = this.getInputValue(item.weight_per_unit, 1);
                //todo stock check
                if (qty_value <= 0) {
                    qty_value = 1;
                }
                $(item.fact_qty).val(qty_value);
                $(item.fact_unit).val((qty_value*weight_per_unit).toFixed(4));
            },

            checkFactCases: function (item) {
                var case_value = this.getInputValue(item.fact_cases, 1);
                //todo stock check
                if (case_value <= 0) {
                    case_value = 1;
                }
                $(item.fact_cases).val(Math.round(case_value));
            },

            checkFactUnit: function (item) {
                var unit_value = this.getInputValue(item.fact_unit, 1);
                var weight_per_unit = this.getInputValue(item.weight_per_unit, 1);
                //todo stock check
                if (unit_value <= 0) {
                    unit_value = 1;
                }
                $(item.fact_unit).val(unit_value.toFixed(4));
                $(item.fact_qty).val((unit_value/weight_per_unit).toFixed(4));
            },
