...
class IntervalManager implements IntervalManagerInterface
{
    ...
    {
        if ($product->getId() > 0) {
            $intervals = $this->intervalRepository->getIntervalsByProductId($product->getId());
    ...
    {
        if ($intervals === null) {
            $searchResults = $this->intervalRepository->getIntervalsByProductId($product->getId());
    ...
    protected function getCustomOptionValues(\Magento\Catalog\Api\Data\ProductCustomOptionInterface $option)
    {
        ...
        $valuesById = [];
        foreach ($option->getValueInstance()->getValues() as $value) {
