...
class Notify
{
    ...
    public function notifyUpcomingSubscriptions() {
        ...
        if (count($subscriptions)) {
            $this->log(__('CRON-notify: Emailing %1 subscriptions.', count($subscriptions)));
            $notified = 0;
            $failed = 0;
            foreach ($subscriptions as $subscription) {
                try {
                    ...
                    $failed++;
                    $this->log(__('CRON-notify: Subscription %1 failed. Error: %2', $subscription->getIncrementId(), $e->getMessage()));
                }
            }
            $this->log(__('CRON-notify: Completed; %1 notified, %2 failed.', $notified, $failed));
        }
        $secondNotice = $this->fetchUpcomingSecondSubscriptions();
        if(count($secondNotice)){
            $this->log(__('CRON-notifiy: Second Emailing %1 subscriptions.', count($secondNotice)));
            $notified = 0;
            $failed = 0;
            foreach($secondNotice as $subscription){
                try {
                    $this->notifySecondSubscription($subscription);
                    $notified++;
                } catch (\Exception $e) {
                    $failed++;
                    $this->log(__('CRON-notify: Second Subscription %1 failed. Error: %2', $subscription->getIncrementId(), $e->getMessage()));
                }
            }
            $this->log(__('CRON-notify: Completed Second; %1 notified, %2 failed.', $notified, $failed));
        }
    }

    protected function fetchUpcomingSecondSubscriptions(){
        $threshold = $this->config->getSecondBillingNoticeAdvance();
        ...
        $subscriptions->addFieldToFilter('next_run', ['gteq' => $start->format(\Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT)]);
        $subscriptions->addFieldToFilter('next_run', ['lteq' => $end->format(\Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT)]);
        $subscriptions->addFieldToFilter('last_notified_two', ['lt' => $lastNotified->format(\Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT)]);
    ...
    protected function notifySecondSubscription(\ParadoxLabs\Subscriptions\Api\Data\SubscriptionInterface $subscription){
        $this->emailSender->sendSecondBillingNoticeEmail($subscription);
        $subscription->setLastNotifiedTwo(time());
        $this->subscriptionRepository->save($subscription);
    }
